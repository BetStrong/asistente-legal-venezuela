    ...
    try {
      ...
      setMessages([
        ...updatedMessages,
        {
          role: 'assistant',
          content:
            'Hubo un error al conectar con el servidor legal. Por favor, intente de nuevo.',
        },
      ]);
    } finally {
      setIsLoading(false);
          ...
          <nav className="space-y-1">
            {TOPICS.map((topic) => (
              <button
                key={topic.name}
                onClick={() =>
                  handleSend(`Consulta sobre derecho ${topic.name.toLowerCase()}`)
                }
        ...
        <header className="p-6 bg-gradient-to-r from-[#001F3F] to-[#0074D9] text-white flex items-center justify-between shadow-lg z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-blue-100/60 uppercase tracking-widest font-medium">
                Consultoría Virtual Especializada
              </p>
            </div>
          </div>
            ...
            >
              ...
              <div className="p-5 rounded-2xl bg-white border-l-[3px] border-[#FFD700] rounded-tl-none shadow-md flex items-center gap-3">
                ...
                <span className="text-sm text-stone-500 font-medium italic">
                  El abogado está redactando su respuesta...
                </span>
              </div>
            </motion.div>
          )}
        ...
        <div className="p-6 bg-white/50 backdrop-blur-md border-t border-stone-200">
          ...
          <div className="mt-4 flex flex-col items-center gap-1">
            ...
            <p className="text-[10px] text-stone-400 max-w-lg text-center leading-tight">
              Este sistema utiliza Inteligencia Artificial para orientación
              informativa. Verifique siempre en Gaceta Oficial. No constituye una
